...
import { FiMapPin, FiBriefcase, FiDollarSign, FiCalendar, FiUsers, FiXCircle, FiCheckCircle, FiSave } from 'react-icons/fi'; // Added FiSave for the save icon
...
export default function JobDetailPage() {
    ...
    const handleSaveUnsaveJob = async () => {
        ...
        try {
            let response;
            if (isCurrentlySaved) {
                // DELETE request to /api/seeker/saved-jobs with jobId as query parameter
                ...
                // POST request to save a new job
            ...
            // After successful operation, re-fetch saved status to ensure consistency
            const updatedIsSaved = await fetchSavedJobStatus(jobId, user?._id);
            setJob(prevJob => prevJob ? { ...prevJob, isSaved: updatedIsSaved } : null);
                        ...
                        <div className="flex-1 bg-white rounded-xl shadow-lg border border-gray-100 p-6 lg:p-8 transform transition-transform duration-300 hover:scale-[1.005]">
                            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
                                ...
                                <div className="text-right sm:self-center">
                                    <span className="text-3xl font-bold text-gray-900">₹{job?.salary.toLocaleString('en-IN') || 'N/A'}</span>
                                    ...
                                    <div className="mb-8">
                                        ...
                                        <ul className="space-y-3 text-gray-700">
                                            ...
                                            <li className="flex items-center">
                                                ...
                                                <strong>Salary:</strong> ₹{job.salary.toLocaleString('en-IN')} / year
                                            </li>
                                            <li className="flex items-center">
                                    ...
                                    {moreJobs.map((otherJob) => (
                                        <Link href={`/seeker/job/${otherJob._id}`} key={otherJob._id} passHref>
                                            <div className="block p-4 rounded-lg border border-gray-200 hover:bg-gray-50 transition-colors duration-200 cursor-pointer shadow-sm">
                                                ...
                                                <p className="text-sm text-gray-500 mt-1">₹{otherJob.salary.toLocaleString('en-IN')}/year</p>
                                            </div>
                                        </Link>
                                    ))}
                                </div>
