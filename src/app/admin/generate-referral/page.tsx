...
import {
  ...
  FiTag,
  FiMenu,
  FiUsers, // For the 'Generated By' filter icon
  // FiSearch, // Removed for the search input
} from "react-icons/fi";
...
interface ReferralCode {
  ...
  generatedByAdminId: string; // Added to store the ID for filtering
...
export default function GenerateReferralPage() {
  ...
  // State for filtering by admin who generated the code
  const [filterGeneratedBy, setFilterGeneratedBy] = useState<string>("all");
  // State to store list of admins and job referrers for the dropdown
  const [adminUsers, setAdminUsers] = useState<{ _id: string; username: string }[]>([]);
  // Removed: State for search query on referral codes
  // const [searchQueryReferrals, setSearchQueryReferrals] = useState<string>('');
  ...
  // Fetch list of admin and job referrer users for the filter dropdown
  const fetchAdminUsers = useCallback(async () => {
    if (!token) return;
    try {
      // Request users with roles 'admin' or 'job_referrer'
      const response = await fetch('/api/admin/users?roles=admin,job_referrer', { // Updated API call to fetch specific roles
      ...
      if (response.ok && Array.isArray(data.users)) {
        ...
        console.error('Failed to fetch admin/referrer users:', data.error);
      }
    } catch (err) {
      console.error('Error fetching admin/referrer users:', err);
    }
  }, [token]);
  ...
  const fetchReferralCodes = useCallback(async () => {
    ...
    try {
      ...
      // Add generatedByAdminId filter if selected
      if (filterGeneratedBy !== 'all') {
        params.append('generatedByAdminId', filterGeneratedBy);
      }
      // Removed: search query parameter
      // if (searchQueryReferrals) {
      //   params.append('search', searchQueryReferrals);
      // }

      // IMPORTANT: Ensure your backend /api/admin/referral-codes endpoint
      // returns ALL referral codes on the platform for an admin, not just those
      // created by the specific admin, when the 'status' and 'generatedByAdminId' filters are applied.
  ...
  }, [token, filterStatus, filterGeneratedBy]); // Removed searchQueryReferrals from dependencies
  ...
  useEffect(() => {
    ...
    // Fetch admin users and referral codes on component mount/dependencies change
    fetchAdminUsers();
    // No debounce needed for search anymore, so call directly or with a small delay for initial load
    fetchReferralCodes();
  }, [authLoading, isAuthenticated, currentUser, router, fetchReferralCodes, fetchAdminUsers, filterStatus, filterGeneratedBy]); // Removed searchQueryReferrals
            ...
            >
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                ...
                <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
                  {" "}
                  {/* Group filters */}
                  {/* Removed: Search Input for Referral Codes */}
                  {/* <div className="relative w-full sm:w-64">
                    ...
                    <input
                      type="text"
                      placeholder="Search codes or candidate emails"
                      value={searchQueryReferrals}
                      onChange={(e) => setSearchQueryReferrals(e.target.value)}
                      className="block w-full bg-white border border-gray-300 text-gray-700 py-2 px-4 pl-10 rounded-lg leading-tight focus:outline-none focus:bg-white focus:border-blue-500 transition-colors shadow-sm"
                    />
                  </div> */}
                  ...
                  <div className="relative w-full sm:w-48">
                    ...
                    <select
                      ...
                      className="block appearance-none w-full bg-white border border-gray-300 text-gray-700 py-2 px-4 pl-10 pr-10 rounded-lg leading-tight focus:outline-none focus:bg-white focus:border-blue-500 transition-colors shadow-sm cursor-pointer"
                    >
                  ...
                  {/* Generated By Admin/Referrer Filter Dropdown */}
                  <div className="relative w-full sm:w-48">
                    <div className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none text-gray-700">
                    ...
                    <select
                      ...
                      className="block appearance-none w-full bg-white border border-gray-300 text-gray-700 py-2 px-4 pl-10 pr-10 rounded-lg leading-tight focus:outline-none focus:bg-white focus:border-blue-500 transition-colors shadow-sm cursor-pointer"
                    >
                      <option value="all">All Admins/Referrers</option>
                ...
                >
                  {generatedReferrals.map((referral) => (
                    ...
                    >
                      ...
                      >
                        <FiTag className="h-4 w-4" /> Status:{" "}
                        {referral.status}
                      </div>
                    </motion.div>
                  ))}
