...
export default function GenerateReferralPage() {
  ...
  const fetchReferralCodes = useCallback(async () => {
    ...
    if (!token) {
      setReferralsError(
        "Authentication token missing. Cannot fetch referral codes."
      );
    ...
    try {
      const response = await fetch(
        `/api/admin/referral-codes?status=${filterStatus}`,
        {
          headers: {
      ...
      setReferralsError(
        err.message || "An error occurred while fetching codes."
      );
    } finally {
      setLoadingReferrals(false);
    }
  ...
  useEffect(() => {
    ...
    if (currentUser.role !== "admin") {
      ...
      else if (currentUser.role === "job_seeker")
